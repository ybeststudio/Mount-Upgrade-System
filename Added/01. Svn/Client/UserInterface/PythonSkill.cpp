...
float CPythonSkill::SSkillData::ProcessFormula(CPoly* pPoly, float fSkillLevel, int iMinMaxType)
{
	if (pPoly->Analyze())
	{
		for (DWORD i = 0; i < pPoly->GetVarCount(); ++i)
		{
			...
			float fState;
			if (!strcmp("SkillPoint", c_szVarName) || !strcmp("k", c_szVarName) || !strcmp("skilllv", c_szVarName))
			{
				fState = fSkillLevel;
			}
			else if (!strcmp("horselv", c_szVarName))
			{
				int iHorseLevel = 0;
				DWORD dwSkillSlotIndex = 0;
				if (CPythonPlayer::Instance().FindSkillSlotIndexBySkillIndex(c_iSkillIndex_Riding, &dwSkillSlotIndex))
				{
					iHorseLevel = CPythonPlayer::Instance().GetSkillLevel(dwSkillSlotIndex);
					const int iGrade = CPythonPlayer::Instance().GetSkillGrade(dwSkillSlotIndex);
					if (iGrade == 1)
						iHorseLevel += 19;
					else if (iGrade == 2)
						iHorseLevel += 29;
