...
void CClientManager::ItemAward(CPeer* peer, char* login)
{
	...
	ItemAwardSet::iterator it = pSet->begin();
	while (it != pSet->end())
	{
		TItemAward* pItemAward = *(it++);
		char* whyStr = pItemAward->szWhy; // why
		char cmdStr[100] = "";
		strcpy(cmdStr, whyStr);
		char command[20] = "";
		GetCommand(cmdStr, command);
		if (!(strcmp(cmdStr, "GIFT")))
		{
...
void CClientManager::RESULT_ITEM_LOAD(CPeer* peer, MYSQL_RES* pRes, DWORD dwHandle, DWORD dwPID)
{
	...
	if (dwCount)
	{
		...
		{
			const auto it = m_map_itemTableByVnum.find(s_items[i].dwVnum);
