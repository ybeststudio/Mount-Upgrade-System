...
bool CHARACTER::StartRiding()
{
	if (IsDead() == true)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_STRING("You cannot get on the horse while you are lying on the ground."));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_STRING("You cannot ride while you are wearing a Wedding Dress or a Tuxedo."));
		return false;
	}
	...
	{
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("You do not have a Horse."));
		else if (GetHorseHealth() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("Your Horse is dead."));
...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pHorseName)
{
	if (bSummon)
	{
		...
		if (!m_chHorse)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("Calling the Horse has failed."));
			return;
		}
