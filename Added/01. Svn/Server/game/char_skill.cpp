...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	*/
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_STRING("You cannot train this skill up to Grand Master level."));
		return false;
	}
	...
	{
		if (GetSkillMasterType(dwSkillVnum) > SKILL_GRAND_MASTER)
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("You already are a Master of this skill. You cannot train this skill any further."));
		else
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("Your Skill is not high enough to become a Grand Master."));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("That did not work. Damn!"));
		ChatPacket(CHAT_TYPE_INFO, LC_STRING("Training has failed. Please try again later."));
		LogManager::instance().CharLog(this, dwSkillVnum, "GM_READ_FAIL", "");
		return false;
	}

	ChatPacket(CHAT_TYPE_TALKING, LC_STRING("My body is full of energy!"));
	ChatPacket(CHAT_TYPE_TALKING, LC_STRING("The training seems to be working already..."));
	ChatPacket(CHAT_TYPE_INFO, LC_STRING("You successfully finished your training with the book."));
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	if (pkSk->dwType != SKILL_BOOK_TYPE_SUPPORT)
	{
		if (GetSkillMasterType(dwSkillVnum) != SKILL_MASTER)
		{
			if (GetSkillMasterType(dwSkillVnum) > SKILL_MASTER)
			{
				ChatPacket(CHAT_TYPE_INFO, LC_STRING("You cannot train this skill with a Book."));
				return false;
			}
			else
			{
			...
			if (pPC)
			{
				...
				if (number(1, 100) > percent)
				{
					if (read_count >= need_bookcount)
					{
						...
						ChatPacket(CHAT_TYPE_INFO, LC_STRING("You have successfully finished your training with the Book."));
					...
					else
					{
						...
						{
							case 1:
								ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I'm making progress, but I still haven't understood everything."));
								break;

							case 2:
								ChatPacket(CHAT_TYPE_TALKING, LC_STRING("These instructions are difficult to understand. I have to carry on studying."));
								break;

							case 3:
							default:
								ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I understand this chapter. But I've got to carry on working hard."));
								break;
						}

						ChatPacket(CHAT_TYPE_INFO, LC_STRING("You have to read %d more skill books to improve this skill.", need_bookcount - read_count));
						return true;
					}
				}
			}
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
		...
		bool bUnderEunhyung = m_pkChr->GetAffectedEunhyung() > 0 ? true : false;
		...
		{
			m_pkChr->ChatPacket(CHAT_TYPE_INFO, "There is no effect. Check your skill formula");
		}
		////////////////////////////////////////////////////////////////////////////////
		iAmount = -iAmount;
...
EVENTFUNC(skill_gwihwan_event)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_STRING("Teleportation has failed."));
	}
	return 0;
}
...
int CHARACTER::ComputeSkillAtPosition(DWORD dwVnum, const PIXEL_POSITION& posTarget, BYTE bSkillLevel)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "There is no effect. Check your skill formula");
	}
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if (dwVnum == SKILL_HORSE_SUMMON)
	{
		...
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("No Horse here. Ask the Stable Boy."));
		else
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("Please use an item to call a Horse."));
	...
	else
	{
		...
		if (test_server)
			ChatPacket(CHAT_TYPE_INFO, LC_STRING("%s FP-Consumption: %d", LC_SKILL(pkSk->dwVnum), iNeededSP));
...
void CHARACTER::SkillLearnWaitMoreTimeMessage(DWORD ms)
{
	// const char* str = "";
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I am burning inside, but it is calming down my body. My Chi has to stabilise."));
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("A little slow, but steady... Without stopping!"));
	else if (ms < 10 * 60) // 10?
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("Yes, that feels great. My body is full of Chi."));
	else if (ms < 30 * 60) // 30?
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I have read it! Now the Chi will spread through my body."));
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("The training is completed."));
	}
	else if (ms < 1 * 3600) // 1
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I am on the last page of the book. The training is nearly finished!"));
	else if (ms < 2 * 3600) // 2
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("Nearly finished! Just a little bit more to go!"));
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("Eureka! I have nearly finished reading it!"));
	else if (ms < 6 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("Only a few more pages and then I'll have read everything."));
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I feel refreshed."));
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("Now I understand it!"));
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("Okay I have to stay concentrated!"));
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I keep reading the same line over and over again."));
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("I do not want to learn any more."));
	}
	else //if (ms < 2 * 86400)
	{
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("It is a lot more complicated and more difficult to understand than I thought."));
		ChatPacket(CHAT_TYPE_TALKING, LC_STRING("It's hard for me to concentrate. I should take a break."));
	}
...
bool CHARACTER::IsUsableSkillMotion(DWORD dwMotionIndex) const
{
	...
	{
		//	ID	ID	ID	ID	(WOLFMAN) ID
		{	0,		0,			0,			0,			0,			0		}, // 0 (RESERVED)

		// SKILL GROUP 1 - TRAINING
		{	5,		1,			31,			61,			91,			170		}, // 1
		{	5,		2,			32,			62,			92,			171		}, // 2
		{	5,		3,			33,			63,			93,			172		}, // 3
		{	5,		4,			34,			64,			94,			173		}, // 4
